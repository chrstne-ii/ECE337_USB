...
module data_buffer #(
    ...
    logic [6:0] write_index, read_index, next_write_index, next_read_index;
    logic [7:0] fifo_in;
    ...
    always_comb begin : NEXT_WRITE_LOGIC
        ...
        if (store_rx_packet_data||store_tx_data) begin
            if (write_index == 7'd64) begin
                next_write_index = 7'd64;
            end else begin
                next_write_index = next_write_index + 1;
            end
        end else if (clear||flush) begin
            next_write_index = 7'd0;
        end
    end
    ...
    always_comb begin : NEXT_FIFO
        ...
        if (buffer_occupancy < 64) begin
            if (store_rx_packet_data||store_tx_data||clear||flush) begin
    ...
    always_comb begin : NEXT_READ_LOGIC
        ...
        if (clear||flush) begin
            next_read_index = 7'd0;
        end else if (get_tx_packet_data||get_rx_data) begin
            if (read_index == 7'd64) begin
                next_read_index = 7'd64;
            end else begin
