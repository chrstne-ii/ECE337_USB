    ...
    always_comb begin
        ...
        casez(state)
            ...
            IDLE: begin
                ...
                hrready = 1;
                hr_avail = 0;
                if(haddr==4'h0 && hsize == 2 && !hresp && !hwrite && hsel) begin
                    next_state = FIRST_S4;
                end 
                if(haddr==4'h1 && hsize == 2 && !hresp && !hwrite && hsel) begin
                    next_state = FIRST_S3;
                end 
                if(haddr==4'h2 || hsize == 1 && !hresp && !hwrite && hsel) begin
                    next_state = FIRST_S2;
                end 
                else if((haddr<4'h4 && hsize == 0 || haddr == 4'h3) && !hresp && !hwrite && hsel)begin
                    next_state = FIRST_S1;
                end
                else begin
                    next_state = IDLE;
