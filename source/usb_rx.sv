...
module usb_rx (
    ...
    logic synced_p, synced_m, signal, new_packet, flag4, first_in, next_in, pserial_in, mserial_in, samp_flag;
    ...
    always_comb begin
        new_packet = synced_p && !signal;
        pserial_in = 1;
        mserial_in = 1;
        enable_shift = 0;
        if(first_in != synced_p) pserial_in = 0;
        if(next_in != synced_m) mserial_in = 0;
        if(samp_flag || samp_count == 5'd8 || samp_count == 5'd17) enable_shift = 1;
    end
